...
│   ├── index.tsx           # 🚀 앱 초기화, 라우트 등록, 서버 실행 (Entry Point)
│   ├── config.ts           # ⚙️ 환경 설정 및 동적 경로(__dirname) 계산
│   ├── utils/              # 💾 파일 시스템(fs) 제어 및 공통 유틸리티
│   ├── components/         # 🎨 Hono JSX 기반 화면 레이아웃 및 조각(Partial) 컴포넌트
│   └── routes/             # 🔗 API 엔드포인트 라우터 컨트롤러
